...
Use is similar to \ref{sec:chapter5bis} explanations: 
\begin{itemize}
\item load pickcell, then load a file image from the application window,
\item load quickmap, then select the quickmap pickcell variant from tool menu, to enable
pickcell from an OpenStreetMap.
\end{itemize}
.}
...
reproducing fire expansions of phenomenonsconsuming vegetation.
...
A general flow to obtain such regions, is (figure \ref{fig:pickcellFlow}) :
\begin{enumerate}
